import JAVASCRIPT from "../public/icons/js.svg";
import CSS from "../public/icons/css.svg";
import HTML from "../public/icons/html.svg";
import REACT from "../public/icons/react.svg";
import SASS from "../public/icons/sass.svg";
import BOOTSTRAP from "../public/icons/bootstrap.svg";
import PYTHON from "../public/icons/python.svg";
import RUBY from "../public/icons/ruby.svg";
import DJANGO from "../public/icons/dj.svg";
import RAILS from "../public/icons/rails.svg";
import NEXTJS from "../public/icons/nextjs.svg";
import MATLAB from "../public/icons/matlab.svg";
import ARDUINO from "../public/icons/arduino.svg";
import WEBPACK from "../public/icons/webpack.svg";
import GIT from "../public/icons/git.svg";
import TAILWIND from "../public/icons/tailwind.svg";
import COLLABORATION from "../public/icons/collaboration.svg";
import REMOTE from "../public/icons/remotework.svg";
import SOFTHARD from "../public/icons/softwarehardware.svg";
import WEBDESIGN from "../public/icons/webdesign.svg";

export const professionalSkill = [
  {
    id: 1,
    alt: "remote work",
    img: REMOTE,
    desc: "We just need an outer wrapper to be relative positioned and hide the overflow, then an inside container.",
  },
  {
    id: 2,
    alt: "collaboration",
    img: COLLABORATION,
    desc: "We just need an outer wrapper to be relative positioned and hide the overflow, then an inside container.",
  },
  {
    id: 3,
    alt: "web design",
    img: WEBDESIGN,
    desc: "We just need an outer wrapper to be relative positioned and hide the overflow, then an inside container.",
  },
  {
    id: 4,
    alt: "software hardware",
    img: SOFTHARD,
    desc: "We just need an outer wrapper to be relative positioned and hide the overflow, then an inside container.",
  },
];

const logos = [
  {
    id: 1,
    alt: "html logo",
    img: HTML,
  },
  {
    id: 2,
    alt: "css logo",
    img: CSS,
  },
  {
    id: 3,
    alt: "sass logo",
    img: SASS,
  },
  {
    id: 4,
    alt: "bootstrap logo",
    img: BOOTSTRAP,
  },
  {
    id: 5,
    alt: "tailwind logo",
    img: TAILWIND,
  },
  {
    id: 6,
    alt: "javascript logo",
    img: JAVASCRIPT,
  },
  {
    id: 7,
    alt: "react logo",
    img: REACT,
  },
  {
    id: 8,
    alt: "next.js logo",
    img: NEXTJS,
  },
  {
    id: 9,
    alt: "ruby logo",
    img: RUBY,
  },
  {
    id: 10,
    alt: "rails logo",
    img: RAILS,
  },
  {
    id: 11,
    alt: "python logo",
    img: PYTHON,
  },

  {
    id: 12,
    alt: "django logo",
    img: DJANGO,
  },

  {
    id: 13,
    alt: "webpack logo",
    img: WEBPACK,
  },

  {
    id: 14,
    alt: "arduino logo",
    img: ARDUINO,
  },

  {
    id: 15,
    alt: "matlab logo",
    img: MATLAB,
  },

  {
    id: 16,
    alt: "git logo",
    img: GIT,
  },
];

export default logos;
